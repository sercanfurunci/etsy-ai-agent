...
_NEGATIVE_ADDITIONS = (
    ...
    "ornamental border, floral border, illustrated border, decorative frame surround, "
    "floral frame, corner ornament, illustrated frame, page border, manuscript border, "
...
- Fix all visual logic errors silently — scan the prompt for the following classes of issues and add corrective spatial/anatomical language without mentioning you changed anything:
  ...
  • Celestial uniqueness: there can only be one sun and one moon in a scene — never two suns, never two moons. This is a hard constraint. Scan the prompt for any phrasing that could produce multiple celestial bodies (dual sunset, twin suns, reflected sun treated as second sun, sunrise glow rendered as a second full orb alongside the moon). If the scene is dawn/dusk and the moon is still visible in the sky, that is allowed — but one must be dominant and the other small/faint: "a faint crescent moon low on the horizon, the dominant light source being the rising sun off-canvas". If both are full glowing orbs of equal size and brightness, that is wrong — reduce to one. Anchor explicitly: "a single full moon, upper right, only celestial body in the sky" or "a single setting sun, centre-right horizon, no moon visible". Add "only one sun" or "only one moon" as a direct instruction in the output prompt if the scene includes sky.
  ...
  • Ice and frozen water patterns: frozen water surfaces (ice, frost, frozen pond) must be rendered as flat, matte, pale blue-grey or white surfaces — not as geometric crystalline lattices, spider-web grids, or angular faceted patterns. These geometric artefacts appear when "ice" or "frost" are described without surface-texture anchoring. Add "frozen surface as smooth flat pale grey-white, no geometric crystal lattice, no angular faceted pattern" if ice or frost is present.
  • Decorative border / illustrated frame: the composition must reach all four canvas edges with no ornamental border, floral frame, or illustrated surround. This is the most critical full-bleed rule. If the prompt contains any word that could produce a decorative border (manuscript, page, scroll edge, illuminated, bordered, framed illustration), remove it and replace with "edge-to-edge composition, no border, no frame, artwork fills all four corners."
